  ...
  render() {
    if (this.state.hasError) return (
      <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#0f172a', color:'white', fontFamily:'system-ui', padding:24, textAlign:'center' }}>
        ...
        <button onClick={() => window.location.href = '/login'} style={{ padding:'12px 24px', background:'#f59e0b', color:'#0f172a', fontWeight:800, borderRadius:12, border:'none', cursor:'pointer', fontSize:14 }}>
          Back to Login
        </button>
      </div>
    )
